...
- **Add: Claude Fable 5.1** — selectable via `/model`; the `fable` shortcut now resolves to it. Fable 5 remains available. Bare `claude-fable-5-1` is 1M (no `[1m]` suffix). If the installed pi-ai snapshot lacks the catalog row, the entry is derived from Fable 5. Needs Claude Code 2.1.251+; point `provider.pathToClaudeCodeExecutable` at a current `claude` if the SDK-bundled CLI is older. `resolveModel` prefers an exact id so `claude-fable-5` does not silently become 5.1.
